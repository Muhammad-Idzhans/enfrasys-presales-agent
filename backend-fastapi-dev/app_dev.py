...
# ---------------------------------------------------------------------
def run_agent(agent_ref: str, user_message: str, timeout_seconds: int = 420) -> dict:    
    """
...
def process_analysis_job(job_id: str, payload: AnalyzeRequest):
    try:
        ...
        """
        agent1_result = run_agent(PRESALES_ANALYST_AGENT, presalesAnalystAgent_input)
        presalesAnalystAgent_output = agent1_result["text"]

        JOBS[job_id]["token_usage"]["presales_analyst"] = {
            "input_tokens": agent1_result["input_tokens"],
            "output_tokens": agent1_result["output_tokens"],
            "cached_tokens": agent1_result["cached_tokens"],
        }
        ...
        ===== END OF ANALYSIS =====
        """
        agent2_result = run_agent(HTML_JSON_FORMATTER_AGENT, htmlJsonFormatterAgent_input)
        htmlJsonFormatterAgent_output = agent2_result["text"]

        JOBS[job_id]["token_usage"]["json_formatter"] = {
            "input_tokens": agent2_result["input_tokens"],
            "output_tokens": agent2_result["output_tokens"],
            "cached_tokens": agent2_result["cached_tokens"],
        }
...
def start_analysis(payload: AnalyzeRequest, background_tasks: BackgroundTasks):
    ...
    JOBS[job_id] = {
        ...
        "error": None,
        "token_usage": {
            "presales_analyst": {"input_tokens": 0, "output_tokens": 0, "cached_tokens": 0},
            "json_formatter":   {"input_tokens": 0, "output_tokens": 0, "cached_tokens": 0},
        },
    }
